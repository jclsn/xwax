...
static inline void pitch_init(struct pitch *p, double dt,
        double q_stable, double r_stable,
        double q_medium, double r_medium,
        double q_reactive, double r_reactive,
...
static inline void pitch_dt_observation(struct pitch *p, double dx)
{
    ...
    if (y_abs > p->scratch_threshold) {
        ...
        pitch_retune_noise(p, &p->q_medium, &p->r_medium); // slightly reactive
        /* printf("medium_mode\n"); */
    } else {
